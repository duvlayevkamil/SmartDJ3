...
def on_progress(class_name, idx, total, score):
    ...
    print(f"   {idx}/{total} {class_name} — ball: {score} — {elapsed:.1f}s — {speed:.2f} sinf/s")
...
# Masshtab
print(f"\nMasshtab: 10 sinf = {elapsed:.1f}s")
print(f"  50 sinf: ~{elapsed/10*50:.0f}s ({elapsed/10*50/60:.1f} daqiqa)")
print(f"  100 sinf: ~{elapsed/10*100:.0f}s ({elapsed/10*100/60:.1f} daqiqa)")
print(f"  250 sinf: ~{elapsed/10*250:.0f}s ({elapsed/10*250/60:.1f} daqiqa)")
